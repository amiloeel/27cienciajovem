...
$sql = "select id, nome, email, z, login from coord where nome like '%$filtro%' or email like '%$filtro%' union all select id, nome, email, z, login from avaliadores where nome like '%$filtro%' or email like '%$filtro%' union all select id, nome, email, z, login from aux where nome like '%$filtro%' or email like '%$filtro%' order by nome";
...
<html lang="pt-br">
	...
	height: 50px;line-height: 50px;background-color: #e34f4f;color: white;margin-top: 5px;box-sizing: border-box;padding-left: 10px;text-align:center;margin-left: 10px;list-style-type: none;">Encerrar Sessão</li></a><br>
			...
			<section class="nova">
				...
				<input type="submit" name="mais" value="+" class="new2" style="float:right; background-color:#2a578b;font-weight:bold;font-size:large;width:30px;margin-top:0px">
				</form>';
