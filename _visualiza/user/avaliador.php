...
$sql = "select id, nome, email, z, login from avaliadores where nome like '%$filtro%' or email like '%$filtro%' order by nome";
...
<html lang="pt-br">
	...
	height: 50px;line-height: 50px;background-color: #e34f4f;color: white;margin-top: 5px;box-sizing: border-box;padding-left: 10px;text-align:center;margin-left: 10px;list-style-type: none;">Encerrar Sessão</li></a><br>
        ...
        if($id!=$nome2){
						...
						$sqlx="select id from res where avaliador1='$nome' union all select id from res where avaliador2='$nome' union all select id from res where avaliador3='$nome'";
						    $sqly="select id from apr where avaliador1='$nome' union all select id from apr where avaliador2='$nome' union all select id from apr where avaliador3='$nome'";
						    
						    $resx=mysqli_query($conexao,$sqlx);
						    $cresx=mysqli_num_rows($resx);
						    $aprx=mysqli_query($conexao,$sqly);
						    $caprx=mysqli_num_rows($aprx);
						    
						    print"<input type=button value='$cresx' style='height:23px;width:23px;background-color:#783d5c;color:whitesmoke;border:none;margin-right:3px;margin-top:5px'>";
						    print"<input type=button value='$caprx' style='height:23px;width:23px;background-color:#783d5c;color:whitesmoke;border:none;'>";
