...
include('../../../_autentica/coord/verifycoord.php');
	...
	if($atv==0){
	    $sql = "update res set fim=1 where id='$id'";
	    $registro = "insert into register (fulano, fez, com) values ('$usuario', 'RES ON', '$nome')";
	}else{
	    $sql = "update res set fim=0 where id='$id'";
	    $registro = "insert into register (fulano, fez, com) values ('$usuario', 'RES OFF', '$nome')";
	}
	...
	if($origem=='base'){
	    header('Location: cruzares.php');
	}else if($origem=='detalhe'){
	    $_SESSION['codex'] = $id;
	    header('Location: detalheres.php');
	}else if($origem=='IP'){
